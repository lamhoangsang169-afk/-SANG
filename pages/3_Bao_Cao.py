...
from database import get_production_logs_by_date_range, get_attendance_db, upload_report_to_storage, save_export_report_db

def render():
    col_rep_h1, col_rep_h2 = st.columns([3, 1])
    with col_rep_h1:
        st.header("3. Báo Cáo & Thống Kê")
    with col_rep_h2:
    ...
    with col_date1: report_start_date = st.date_input("Từ ngày", default_start)
    with col_date2: report_end_date = st.date_input("Đến ngày", default_end)
