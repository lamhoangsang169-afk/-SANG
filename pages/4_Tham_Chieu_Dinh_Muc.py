...
def render(current_user_role, user_perms):
    ...
    with col_rules_h1:
        st.header("4. Tham Chiếu Định Mức Công Việc")
    with col_rules_h2:
    ...
    if current_user_role != "Admin" and not user_perms.get("perm_rules", False):
        ...
        with st.form("rules_form"):
            ...
            with col_save_rule: saved_clicked = st.form_submit_button("💾 Lưu Thay Đổi Định Mức", use_container_width=True)
            with col_clear_rule: clear_clicked = st.form_submit_button("🔥 Xóa Toàn Bộ Định Mức", use_container_width=True)
