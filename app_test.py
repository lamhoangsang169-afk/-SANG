...
def render_main_content(current_menu_name):
    ...
    if feature == "input_production":
        ...
        with f_col1:
            all_dates = ["Tất cả", today_str]
            filter_date = st.selectbox("Lọc theo Ngày", all_dates, index=1 if today_str in all_dates else 0, key="select_filter_date")
        ...
        st.markdown(f"<small style='color: #1d4ed8; font-weight: bold;'>📅 Bộ lọc này có: {count_by_date} bản ghi</small>", unsafe_allow_html=True)
        ...
        if current_user_role != "Admin":
            ...
            trash_df = get_production_logs_db(is_deleted=True, limit_rows=200)
