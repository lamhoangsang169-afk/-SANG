...
def init_session_state():
    ...
    if "play_checkin_audio" not in st.session_state:
        st.session_state.play_checkin_audio = False
...
def render_app():
    ...
    # Phát âm thanh chúc mừng check-in thành công nếu cờ được bật
    if st.session_state.get("play_checkin_audio", False):
        try:
            wish_text = "Chúc bạn hoàn thành tốt công việc, đạt hiệu suất cao!"
            tts = gTTS(text=wish_text, lang='vi', slow=False)
            fp = io.BytesIO()
            tts.write_to_fp(fp)
            fp.seek(0)
    ...
    if menu == "1. Nhập Sản Lượng":
        ...
        with st.form("attendance_form"):
            ...
            if check_in_clicked:
                ...
                # Bật cờ phát âm thanh khi Check-in thành công
                st.session_state.play_checkin_audio = True
