...
def save_staff_list_db(edited_df):
    ...
    try:
        ...
        st.cache_data.clear()
    except Exception as e: st.error(f"Lỗi lưu nhân sự: {e}")
...
def save_app_settings_db(settings_dict):
    if supabase is None: return
    try:
        supabase.table("app_settings").upsert({"id": 1, **settings_dict}).execute()
        st.cache_data.clear()
    except Exception: pass
...
def save_folders_db(folders_list):
    if supabase is None: return
    try:
        supabase.table("app_folders").upsert({"id": 1, "folders_json": folders_list}).execute()
        st.cache_data.clear()
    except Exception: pass
...
def upload_report_to_storage(file_name, csv_bytes):
    ...
    try:
        ...
        return f"{SUPABASE_URL_VAL}/storage/v1/object/public/reports-storage/{unique_file_name}"
        ...
        with tab_admin:
            with st.form("login_admin_form"):
                ...
                if st.form_submit_button("🚀 Đăng Nhập Quản Trị Viên", use_container_width=True):
                    if not email_input or not password_admin: st.error("⚠️ Vui lòng nhập đầy đủ!")
                        ...
                        try:
                            ...
                            if res and res.user:
                                ...
                                st.rerun()
                            else: st.error("❌ Mật khẩu hoặc Email sai!")
...
with st.sidebar:
    ...
    st.markdown(f'<small>🧠 RAM: <b>{ram_usage_str}</b> | 🗄️ CSDL: <b>{db_usage_str}</b> | 💾 Lữu trữ: <b>{storage_usage_str}</b></small>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
def render_main_content(current_menu_name):
    ...
    if feature == "input_production":
        ...
        if current_user_role != "Admin" and not user_perms["perm_input"]:
            ...
            if not active_staff:
                st.warning("⚠️ Hiện tại chưa có nhân sự nào **Check-in (Vào ca)**!")
            else:
                        ...
                        if urls:
                            ...
                            for i, u in enumerate(urls):
                                with sub_cols[i]:
                                    ...
                                    st.image(u, width=40)
                        else: st.markdown("<small style='color: gray;'>Không ảnh</small>", unsafe_allow_html=True)
                    else: st.markdown("<small style='color: gray;'>Không ảnh</small>", unsafe_allow_html=True)
                st.markdown("---")
        else: st.info("Chưa có dữ liệu sản lượng.")
