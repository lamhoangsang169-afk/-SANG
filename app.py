...
# CSS toàn cục: Áp dụng màu chữ tùy chỉnh cho tất cả các thành phần giao diện, bảng dữ liệu và tiêu đề
st.markdown(f"""
<style>
    .stApp {{
        {bg_style}
    ...
    /* Áp dụng màu chữ cho tất cả thẻ văn bản, nhãn, đoạn văn, tiêu đề phụ */
    h2, h3, h4, h5, h6, .stMarkdown, p, span, label, div, [data-testid="stMarkdownContainer"] * {{
        color: {st.session_state.text_color} !important;
    }}
    
    /* Tiêu đề chính giữ màu chủ đạo đặc biệt hoặc theo màu chữ tùy chọn */
    h1 {{
    ...
    /* Đồng bộ màu chữ cho các bảng dữ liệu (data editor / dataframe) */
